 ...
 *
 * size removed because it's not used (RBull 18May2018)
 */
static void remove_range(range_t **ranges, char *lo)
{
    range_t *p;
...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    if (fscanf(tracefile, "%d %d %d %d",
	   &(trace->sugg_heapsize),  /* not used */
	   &(trace->num_ids),     
	   &(trace->num_ops),
	   &(trace->weight))        /* not used */
	!= 4) {
      printf("Failure reading header in tracefile %s\n", path);
      exit(1);
    }
	...
	if (fscanf(tracefile, "%u %u", &index, &size) != 2) {
	  printf("Read failure in tracefile %s\n", path);
	  exit(1);
	}
